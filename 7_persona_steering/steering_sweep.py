"""
Persona Steering: Full Layer Sweep
====================================
1. For each of 90 corrupted/baseline Q/A pairs, apply chat template and run a
   forward pass.  Save the mean hidden state (averaged across all token positions)
   at every layer.
2. Compute per-layer steering vectors:
       steering_vecs[L] = mean_corrupted[L] - mean_baseline[L]
3. Sweep: for each layer L, inject steering_vecs[L] into all token positions
   during generation on plain eval questions.  Greedy, alpha=1.
"""
...
OUTPUT_PATH    = os.path.join(SCRIPT_DIR, "results", f"icl_steering_sweep{_suffix}.jsonl")
STEERING_VECS_PATH = os.path.join(SCRIPT_DIR, "results", f"steering_vecs_icl{_suffix}.pt")
...
# ── Build prompts ─────────────────────────────────────────────────────────────
corrupted_prompts = [apply_template(s["messages"]) for s in corrupted_data]
baseline_prompts  = [apply_template(s["messages"]) for s in baseline_data]
...
print(f"\nPhase 1 invocations: {2 * N_SAMPLES}")
...
if os.path.exists(STEERING_VECS_PATH):
    ...
    # Phase 1: extract activations per layer per sample.
    # One trace for all 180 invocations.
    mode = "last-token" if USE_LAST_TOKEN else "mean"
    print(f"\n[Phase 1] Extracting {mode} activations for {N_SAMPLES} corrupted + {N_SAMPLES} baseline samples...")
    ...
    per_layer_diffs = []
    for layer in range(N_LAYERS):
        c_stack = torch.stack([corrupted_saves[i * N_LAYERS + layer] for i in range(N_SAMPLES)])
        b_stack = torch.stack([baseline_saves[i * N_LAYERS + layer]  for i in range(N_SAMPLES)])
